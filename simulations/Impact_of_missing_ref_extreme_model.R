##Simulation to assess the robustness of theta1 and theta2 when the deconvolution approach ##
## is performed using an incomplete reference panel (extreme model)##
...
Sim_deconv_missing_ref_extreme <- function(M, Pi_s, Fst, N_s, N1_r, C=0.01){
  ...
  # Here we asssume that the second reference (p2_s3) is absent 
  ...
  Pi_s_hat <- coef(nnls(cbind(p1_r2), p_s2))[1]
  
  #weighted reference using estimated proportion for the available reference
  p_r   <- Pi_s_hat * p1_r 
  ...
  var_pr <- (Pi_s_hat^2) * p1_r * (1 - p1_r)/(2*N1_r)  
...
simulations <- run_simulations(Design, Sim_deconv_missing_ref_extreme)
...
# ============================================================
{
  summarise_estimates <- function(dt, value_cols, by_cols = c("Pi_s", "N1_r", "Fst")) {
  ...
  summarise_type1_error <- function(dt, p_cols, alpha = 0.05, by_cols = c("Pi_s", "N1_r", "Fst")) {
  ...
  theta_est_plot <- bind_rows(
    transmute(theta_summary, Pi_s, N1_r, Fst, stat = "theta1", value = theta1, se = theta1_se),
    transmute(theta_summary, Pi_s, N1_r, Fst, stat = "theta2", value = theta2, se = theta2_se)
  ) %>%
  ...
  theta2int_est_plot <- theta2int_summary %>%
    transmute(Pi_s, N1_r, Fst, stat = "theta2_int", value = theta2_int, se = theta2_int_se) %>%
